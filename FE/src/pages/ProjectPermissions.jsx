...
export default function ProjectPermissions() {
  ...
  useEffect(() => {
    ...
    // Deep clone to avoid mutating original matrixData when toggling
  ...
  const toggleAction = (resName, actName) => {
    // Prevent toggling locked rules in UI (frontend guard)
  ...
  const save = async () => {
    try {
      ...
      // Sync matrixData in-memory so isDirty resets and UI stays in sync
      setMatrixData((prev) => {
        if (!prev) return prev;
  ...
  if (!currentProject) return <MainLayout><div>Chọn project để quản lý phân quyền.</div></MainLayout>;
    ...
    <MainLayout>
      <div className="mt-6 bg-white rounded-2xl shadow p-6">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-5">
          <h2 className="text-xl font-semibold text-emerald-700">Permissions</h2>
                  ...
                  {open && (
                    <div className="mt-3 grid gap-2">
                      {Object.keys(resActions).map((act) => {
                        ...
                        // Nhãn hiển thị gợi ý: "View Sprints", "Add Sprints", ...
